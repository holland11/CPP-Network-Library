...
void net_server::send_to(std::size_t id, const char* body, std::size_t length) {
	...
	for (auto& connection : connections_) {
		if (connection->get_id() == id) {
			if (!connection->valid()) {
				std::cerr << "attempting to send a message to an invalid connection object" << std::endl;
				return;
			}
	...
	std::cerr << "attempted to send_to client " << id << ", but no connection exists with that id." << std::endl;
}
